...
export function handleBoughtFromAmmEvent(event: BoughtFromAmm): void {
  ...
  if (position !== null) {
    let userBalanceTo = PositionBalance.load(event.params.asset.toHex() + ' - ' + event.params.buyer.toHex());
    if (userBalanceTo === null) {
      userBalanceTo = new PositionBalance(event.params.asset.toHex() + ' - ' + event.params.buyer.toHex());
...
export function handleSoldToAMMEvent(event: SoldToAMM): void {
  ...
  let position = Position.load(event.params.asset.toHex());
  if (position !== null) {
    let userBalanceTo = PositionBalance.load(event.params.asset.toHex() + ' - ' + event.params.seller.toHex());
    if (userBalanceTo === null) {
      userBalanceTo = new PositionBalance(event.params.asset.toHex() + ' - ' + event.params.seller.toHex());
