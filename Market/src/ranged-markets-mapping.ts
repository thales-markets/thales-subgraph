...
export function handleBoughtFromAmmEvent(event: BoughtFromAmm): void {
  ...
  if (position !== null) {
    let userBalanceTo = RangedPositionBalance.load(event.params.asset.toHex() + ' - ' + event.params.buyer.toHex());
    if (userBalanceTo === null) {
      userBalanceTo = new RangedPositionBalance(event.params.asset.toHex() + ' - ' + event.params.buyer.toHex());
      userBalanceTo.account = event.params.buyer;
      userBalanceTo.amount = BigInt.fromI32(0);
      userBalanceTo.position = position.id;
    }
    userBalanceTo.amount = userBalanceTo.amount.plus(event.params.amount);
    userBalanceTo.save();
  }
}
...
export function handleSoldToAMMEvent(event: SoldToAMM): void {
  ...
  let position = RangedPosition.load(event.params.asset.toHex());
  if (position !== null) {
    let userBalanceTo = RangedPositionBalance.load(event.params.asset.toHex() + ' - ' + event.params.seller.toHex());
    if (userBalanceTo === null) {
      userBalanceTo = new RangedPositionBalance(event.params.asset.toHex() + ' - ' + event.params.seller.toHex());
      userBalanceTo.account = event.params.seller;
      userBalanceTo.amount = BigInt.fromI32(0);
      userBalanceTo.position = position.id;
    }
    userBalanceTo.amount = userBalanceTo.amount.minus(event.params.amount);
    userBalanceTo.save();
  }
}
...
export function handleExercised(event: Exercised): void {
  ...
  let marketEntity = RangedMarket.load(event.address.toHex());

  if (marketEntity !== null) {
    let positionIn = RangedPosition.load(marketEntity.inAddress.toHex());
    if (positionIn !== null) {
      let userBalanceUp = RangedPositionBalance.load(
        marketEntity.inAddress.toHex() + ' - ' + event.params.exerciser.toHex(),
      );
      if (userBalanceUp === null) {
        userBalanceUp = new RangedPositionBalance(
          marketEntity.inAddress.toHex() + ' - ' + event.params.exerciser.toHex(),
        );
        userBalanceUp.account = event.params.exerciser;
        userBalanceUp.amount = BigInt.fromI32(0);
        userBalanceUp.position = positionIn.id;
      }
      userBalanceUp.amount = BigInt.fromI32(0);
      userBalanceUp.save();
    }

    let positionOut = RangedPosition.load(marketEntity.inAddress.toHex());
    if (positionOut !== null) {
      let userBalanceDown = RangedPositionBalance.load(
        marketEntity.outAddress.toHex() + ' - ' + event.params.exerciser.toHex(),
      );
      if (userBalanceDown === null) {
        userBalanceDown = new RangedPositionBalance(
          marketEntity.outAddress.toHex() + ' - ' + event.params.exerciser.toHex(),
        );
        userBalanceDown.account = event.params.exerciser;
        userBalanceDown.amount = BigInt.fromI32(0);
        userBalanceDown.position = positionOut.id;
      }
      userBalanceDown.amount = BigInt.fromI32(0);
