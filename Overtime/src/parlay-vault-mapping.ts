...
import {
  ...
  WithdrawalRequested,
  Claimed,
} from '../generated/ParlayDiscountVault/ParlayVault';
...
export function handleVaultTrade(event: TradeExecuted): void {
  let transaction = new ParlayVaultTransaction(event.transaction.hash.toHexString() + '-' + event.logIndex.toString());
    ...
    transaction.market = event.params.parlayMarket;
    transaction.paid = event.params.sUSDPaid;

    let market = ParlayMarket.load(event.params.parlayMarket.toHex());
    if(market !== null) {
